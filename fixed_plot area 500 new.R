...
all_preds <- cbind(plots$bio500, plots$code, dem_ex, loc_min_ex, ndvi_ex, savi_ex, mainbands_ex, results, tex_sm_ex)
#all_preds <- cbind(plots$bio500, dem_ex, loc_min_ex, ndvi_ex, results, tcup_ex, tex_pca_ex, tex_sm_ex)
...
save(all_preds, file="all_preds_extracted_plot500.RData")
load("all_preds_extracted_plot500.RData")

# quick test using all predictors (except for species as
rf_test <- randomForest(all_preds[,-c((1:2))], all_preds[,1], ntree=500, mtry=3)


# run variable selection with VSURF
set.seed(25)
rf_vsurf <- VSURF(all_preds[,-c(1,2)], all_preds[,1], stepFactor=1, doBest=T)
rf_vsurf$varselect.thres
rf_vsurf$varselect.interp
rf_vsurf$varselect.pred
common_predictors_in_all_plot_size <- all_preds[,-c((1:2),(4:7),(10),(15:18),(21:25))]
colnames(all_preds[,-c((1:2),(4:7),(10),(15:18),(21:25))])

# check vsurf results and select predictors

# run final RF model with selected predictors

rf_test <- tuneRF(all_preds[,-c((1:2),(4:7),(10),(15:18),(21:25))], all_preds[,1], stepFactor=1, doBest=T)
colnames(all_preds[,(rf_vsurf$varselect.interp+2)])

# check results
rf_test
plot(all_preds[,1], rf_test$predicted)
abline(0,1, col="red")
...
sp1 <- as.data.frame(all_preds[all_preds[,2]==1,])
sp2 <- as.data.frame(all_preds[all_preds[,2]==2,])
sp3 <- as.data.frame(all_preds[all_preds[,2]==3,])
sp4 <- as.data.frame(all_preds[all_preds[,2]==4,])
...
# create empty matrix to store results
res_fixed_500 <- matrix(NA, ncol=18, nrow=(length(seq(6000,18000,1500))*1000))
...
for (i in seq(6000,18000,1500)){
  i4=500
  # divide the fixed area by 500 (as the plot size is 500 m2)
  ...
  for (i2 in 1:1000){
    ...
    res_fixed_500[dummy,] <- c(i3, i2, i, i4, spearm, spearm2, rmse, rmse2, Rrmse1, Rrmse2, Nrmse1, Nrmse2, Bias1, Bias2,min_b, max_b,min_b2, max_b2)
...
setwd("E:/6_clean_version_new/6_clean_version/99_outputs")
save(res_fixed_500, file = "fixed_area_results_500m.RData")
...
# ## r2
meanr2_500 <-  means_fin[,tar]
sdr2_500 <- sds_fin[,tar]
x <- c(1:9,9:1)
y1 <- meanr2_500+sdr2_500
y2 <-meanr2_500-sdr2_500
y <- c(y1, rev(y2)) 
plot(meanr2_500, col="white", xlim=c(0,10), ylim=c(0,1),ann=F, axes=F)
polygon(x,y, border=NA, col=rgb(0.0, 0.4, 0.1, 0.5), ylim=c(-20,20))
par(new=T)
plot(x[1:9], meanr2_500,ylab="mean r_squared", xlab="plot size",xlim=c(0,10), ylim=c(0,1), pch=1,cex.lab=1.5, cex.axis=1.5)
