...
rf_test <- randomForest(all_preds[,-c((1:2),(4:7),(10),(15:18),(21:25))], all_preds[,1], ntree=500, mtry=3)
...
res_no_fixed_area_500 <- matrix(NA, ncol=18, nrow=(length(seq(2,16,1))*1000))
...
for (i in seq(2,16,1)){
  i4=500
  i5=i*12
  # start second loop and run each sample size 500 times
  for (i2 in 1:1000){
    ...
    res_no_fixed_area_500[dummy,] <- c(i, i2, i5, i4, spearm, spearm2, rmse, rmse2, Rrmse1, Rrmse2, Nrmse1, Nrmse2, Bias1, Bias2, min_b, max_b,min_b2, max_b2)
...
head(res_no_fixed_area_500)

# save model results to file
setwd("E:/6_clean_version_new/6_clean_version/99_outputs")
save(res_no_fixed_area_500, file = "no_fixed_model_results_500m.RData")
...
for (i in vals){
  ...
  sub <- res_no_fixed_area_500[res_no_fixed_area_500[,1]==i,]
  # calculate mean
